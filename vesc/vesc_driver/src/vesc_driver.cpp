...
VescDriver::VescDriver(ros::NodeHandle nh,
  ...
  driver_mode_(MODE_INITIALIZING), fw_version_major_(-1), fw_version_minor_(-1), joy_stop_button_pressed_(false)
{
  ...
  // joy_sub_ = nh.subscribe("/joy", 10, &VescDriver::joyCallback, this);
...
void VescDriver::timerCallback(const ros::TimerEvent& event)
{
  ...
  else if (driver_mode_ == MODE_STOP){
    if (joy_stop_button_pressed_){
...
void VescDriver::joyCallback(const sensor_msgs::Joy& joy){
  if (joy.buttons[0] == 1){
    joy_stop_button_pressed_ = true;
    driver_mode_ = MODE_STOP;
  }
  else{
    joy_stop_button_pressed_ = false;
  }
}
